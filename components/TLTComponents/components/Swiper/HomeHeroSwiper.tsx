        ...
        {swiperData?.map((item, index) => (
          <SwiperSlide key={`index-${index + 1}`}>
            <Box sx={{
              ...
              <Box sx={{
                position: 'absolute',
                top: { xs: index === 0 ? '36%' : '50%' },
                left: { xs: index === 0 ? '61%' : '50%', lg: '50%', xls: index === 0 ? '61%' : '50%' },
                right: { xs: 'unset', lg: index === 0 ? '100px' : 'unset' },
                transform: {
                  xs: index === 0 ? '' : 'translate(-50%,-50%)', md: index === 0 ? '' : 'translate(-50%,-50%)',
                },
              }
              } >
                {index === 0 ? (
